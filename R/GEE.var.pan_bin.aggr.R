  ...
  function(formula,id,data,corstr="independence"){
    ...
    for (i in 1:size){
      ...
      if (family=="gaussian"){
        ...
        resid<-(y-N*exp(covariate%*%beta_est)/(1+exp(covariate%*%beta_est)))%*%
                t(y-N*exp(covariate%*%beta_est)/(1+exp(covariate%*%beta_est)))    ## N* added
        B<-matrix(0,nrow=cluster$n[i],ncol=cluster$n[i])
    ...
    return(list(cov.beta=diag(cov.beta), cov.var=cov.var, tmp=resid))
  }